...
def load_data_from_csv():
    try:
        ...
        # Eliminar columnas de índice previas
...
# NUEVO: FILTRO MULTI-RAREZA
rareza_opciones = sorted(st.session_state.df['rarity'].unique().tolist())
f_rareza = st.sidebar.multiselect("Filtrar por Rareza(s)", options=rareza_opciones)
...
f_unicos = st.sidebar.toggle("🚫 Colapsar duplicados", value=True)
...
# --- REPORTES ---
st.sidebar.divider()
if st.sidebar.button("📦 Lista Adquiridas", use_container_width=True):
    st.session_state.df_result = st.session_state.df[st.session_state.df['cantidad'] > 0].drop_duplicates('name_clean')
    st.session_state.view_mode = "reporte"
if st.sidebar.button("📋 Wishlist", use_container_width=True):
    stock_total = st.session_state.df.groupby('name_clean')['cantidad'].sum()
    n_cero = stock_total[stock_total == 0].index
    st.session_state.df_result = st.session_state.df[st.session_state.df['name_clean'].isin(n_cero)].drop_duplicates('name_clean')
    st.session_state.view_mode = "reporte"
...
if mode == "inicio":
    ...
    st.header("📋 Listado Generado")
    pdf_bytes = generar_pdf(st.session_state.df_result, "Reporte")
    ...
    # Lógica de Filtrado por Múltiples Rarezas
    if f_rareza:
        df_f = df_f[df_f['rarity'].isin(f_rareza)]
    
    if f_unicos:
        df_f = df_f.drop_duplicates(subset=['name_clean'])
    
    total_enc = len(df_f)
    st.write(f"Viendo **{min(st.session_state.items_a_mostrar, total_enc)}** de **{total_enc}** cartas físicas.")
    ...
    for idx, row in df_parcial.iterrows():
        ...
        with c3:
            # STOCK GLOBAL (Suma de todas las versiones)
            ...
            if nuevo_val != stock_global:
                ...
                # Buscamos la primera versión física en el dataframe para aplicarle la diferencia
